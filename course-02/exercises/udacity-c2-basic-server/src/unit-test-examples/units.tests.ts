...
describe("concat", () => {
  it("should throw an error if the first string is empty", () => {
    expect(() => {
      concat("", "hey");
    }).to.throw("Your first String is Empty");
  });

  it("should throw an error if the second string is empty", () => {
    expect(() => {
      concat("hey", "");
    }).to.throw("Your second String is Empty");
  });

  it("should combine 2 strings", () => {
    const result = concat("hey", " sup?");
    expect(result).to.equal("hey sup?");
  });
});
